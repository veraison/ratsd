...
func TestRatsdChares_valid_request(t *testing.T) {
	...
	assert.Equal(t, validNonce, out["eat_nonce"])
	assert.Contains(t, out, "cmw")
	assert.Contains(t, out["cmw"], "mock-tsm")

	content := out["cmw"].(map[string]interface{})["mock-tsm"]
	c := cmw.CMW{}
	c.UnmarshalJSON([]byte(content.(string)))
	assert.Equal(t, c.GetType(), "application/vnd.veraison.mock-tsm+json")

	blob, _ := base64.StdEncoding.DecodeString(string(c.GetValue()))

	var tsmout map[string]string
	json.Unmarshal(blob, &tsmout)
	assert.Equal(t, "fake\n", tsmout["provider"])
}
