...
func Test_GPUEvidence_Valid_Fail_MissingNonce(t *testing.T) {
	...
	assert.EqualError(t, evidence.Valid(), `missing mandatory field "devices[0].nonce"`)
}
...
func Test_GPUEvidence_Valid_Fail_MissingCertificateChain(t *testing.T) {
	...
	assert.EqualError(t, evidence.Valid(), `missing mandatory field "devices[0].certificate"`)
}
